...
impl Engine {
    ...
    pub fn process(&mut self, data: SensorData) -> Vec<SensorData> {
        ...
        if let Err(e) = self.context.set_value(data.sensor_id.clone(), data.value.into()) {
...
mod tests {
    ...
    fn test_simple_calculation() {
        ...
        // 1. Send Input
        let input = SensorData { sensor_id: "Input".to_string(), value: 10.0, timestamp_ms: 100, quality: 1 };
        let res1 = engine.process(input);
        assert_eq!(res1.len(), 1); // Only Input, Efficiency fails (Output missing)

        // 2. Send Output
        let output = SensorData { sensor_id: "Output".to_string(), value: 80.0, timestamp_ms: 100, quality: 1 };
